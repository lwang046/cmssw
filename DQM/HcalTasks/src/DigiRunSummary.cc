  ...
  /* virtual */ void DigiRunSummary::endLuminosityBlock(DQMStore::IBooker& ib,
    ...
    for (std::vector<uint32_t>::const_iterator it = _vhashCrates.begin(); it != _vhashCrates.end(); ++it) {
      ...
      if ((did.subdet() == HcalBarrel) || (did.subdet() == HcalEndcap) || (did.subdet() == HcalOuter) ||
        ...
        std::string pinDiodeHistName = _subsystem + "/" + _taskname + "/PinDiodeMon/SumQvsLS";
      ...
      // Raddam CU (not subdet-split): presence in CU_Raddam_CUCountvsLS
      {
        std::string raddamHistName = _subsystem + "/" + _taskname + "/CU_Raddam/CU_Raddam_CUCountvsLS";
