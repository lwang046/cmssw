...
class OnlineDQMDigiAD
{
  public:
       ...
       */
      std::vector<std::vector<float>> Inference(std::vector<float> &digiHcalMapTW, const std::vector<float> &numEvents, const std::vector<float> &adThr, std::vector<float> &input_model_state_memory_e_0_0, std::vector<float> &input_model_state_memory_e_0_1, std::vector<float> &input_model_state_memory_e_1_0, std::vector<float> &input_model_state_memory_e_1_1,
      ...
      std::vector<std::vector<float>> Inference_CMSSW(const std::vector<std::vector<float>> &digiHcal2DHist_depth_1,
                           ...
                           const std::vector<std::vector<float>> &digiHcal2DHist_depth_7, const float LS_numEvents, const float flagDecisionThr=20);
  ...
  private:
    ...
    //   const unsigned int layer_dims = {{128, 32}, {128, 640}}; // do not change, encoder[layer_0, layer_1] and decoder [layer_0, layer_1]
      const std::vector<std::vector<unsigned int>> layer_dims = {{256, 64}, {256, 1280}}; // do not change, default_model_batch_dim*encoder[layer_0, layer_1] and default_model_batch_dim*decoder [layer_0, layer_1]
     //   const unsigned int default_model_batch_dim = 2;              // do not change
      ...
      unsigned model_state_refresh_counter = 1;                // DQM multithread returns non-sequential LS. Hence, the model will not keep states (experimental)
